...
class CrawlingActorElement extends ParrotElementImpl {
    ...
    private void setSwimAmount(float value) {
        try {
            java.lang.reflect.Field swimAmount = net.minecraft.world.entity.LivingEntity.class.getDeclaredField("swimAmount");
            java.lang.reflect.Field swimAmountO = net.minecraft.world.entity.LivingEntity.class.getDeclaredField("swimAmountO");
            swimAmount.setAccessible(true);
            swimAmountO.setAccessible(true);
            swimAmount.setFloat(skeleton, value);
            swimAmountO.setFloat(skeleton, value);
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger("CardboardedPonder")
                .error("Failed to set swimAmount: {}", e.getMessage(), e);
        }
    }
    ...
    private static ItemStack dyedLeather(Item item) {
        ...
        stack.set(net.minecraft.core.component.DataComponents.DYED_COLOR,
            new net.minecraft.world.item.component.DyedItemColor(LEATHER_COLOR, true));
        return stack;
    }
}
